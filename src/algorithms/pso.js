...
export function repairAllocation(position, villages = defaultVillages, resources = defaultResources) {
  ...
  const x = position.map(roundNonNegative)
  for (let i = 0; i < size; i++) {
    x[i] = Math.min(x[i], villages[i].foodDemand)
    x[size + i] = Math.min(x[size + i], villages[i].medicineDemand)
    x[2 * size + i] = Math.min(x[2 * size + i], Math.ceil((villages[i].foodDemand + villages[i].medicineDemand) / resources.maxPayload))
    x[3 * size + i] = Math.min(x[3 * size + i], resources.batteries)
  }
  capGroup(x, 0, size, resources.food); capGroup(x, size, size, resources.medicine); capGroup(x, 3 * size, size, resources.batteries)
  ...
  villages.forEach((_, i) => { x[2 * size + i] = Math.max(x[2 * size + i], Math.ceil((x[i] + x[size + i]) / resources.maxPayload)) })
  villages.forEach((v, i) => { if (v.distance >= 10 && x[2 * size + i] === 0 && (x[i] + x[size + i]) > 0) x[2 * size + i] = 1 })
  return x
}
...
export function evaluateAllocation(vector, villages = defaultVillages, resources = defaultResources) {
  ...
  const food = x.slice(0, size), medicine = x.slice(size, 2 * size), missions = x.slice(2 * size, 3 * size), batteries = x.slice(3 * size, 4 * size)
  ...
  const satisfaction = villages.map((v, i) => {
    ...
    const neededMissions = Math.ceil((food[i] + medicine[i]) / resources.maxPayload)
  ...
  return {
    vector: x, food, medicine, missions, batteries, cost, fitness: -cost, unmetFood, unmetMedicine,
...
export function runPSO(options = {}) {
  ...
  const max = [...villages.map((v) => v.foodDemand), ...villages.map((v) => v.medicineDemand), ...villages.map((v) => Math.ceil((v.foodDemand + v.medicineDemand) / resources.maxPayload)), ...villages.map(() => resources.batteries)]
