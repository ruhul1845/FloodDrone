...
export function evaluatePolicy(environment, policyFn, { episodes = 200, seed = 99 } = {}) {
  ...
  for (let i = 0; i < episodes; i++) {
    ...
    rows.push({ totalReward, success: state.success ? 1 : 0, criticalSuccess: state.success && target.startsWith('V3') && payload !== 'FOOD' ? 1 : 0, critical: target.startsWith('V3') && payload !== 'FOOD', foodSuccess: state.success && payload === 'FOOD' ? 1 : 0, food: payload === 'FOOD', steps: step, battery: state.battery, batteryFailure: state.battery === 0 && !state.success ? 1 : 0, invalid, severe })
  }
